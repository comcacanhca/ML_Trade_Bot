...
SUMMARY_PATH = Path(__file__).resolve().parent / "outputs" / "transform_until_best_summary.json"


CONFIGS = [
    {
        "name": "products_top10_t50",
        "variants": ["products"],
        "n_trials": 50,
        "top_n": 10,
        "seed": 92101,
    ...
    {
        "name": "mixed_compact_top10_t50",
        "variants": ["mixed_compact"],
        "n_trials": 50,
        "top_n": 10,
        "seed": 92107,
    },
    {
        "name": "mixed_compact_top18_t50",
        "variants": ["mixed_compact"],
        "n_trials": 50,
        "top_n": 18,
        "seed": 92108,
...
def main() -> None:
    ...
    for cfg in CONFIGS:
        ...
        if float(best["test_wr"]) > TARGET_TEST_WR and int(best["test_resolved"]) >= TARGET_MIN_DEALS:
            ...
            return
    print({"phase": "target_not_beaten", "target_test_wr": TARGET_TEST_WR, "best": max(results, key=lambda r: r["test_wr"]) if results else None}, flush=True)
