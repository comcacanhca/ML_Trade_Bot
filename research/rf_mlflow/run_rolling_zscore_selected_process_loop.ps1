param(
    ...
    [string]$Indexes = "17,32,21,7,35,9,2,44,13,48",
    [int]$MaxTrainRows = 180000,
...
$LogPath = Join-Path $WorkDir "rolling_zscore_top_valid_auc_process_loop.log"
$ErrPath = Join-Path $WorkDir "rolling_zscore_top_valid_auc_process_loop.err.log"
$IndexList = $Indexes.Split(",") | ForEach-Object { [int]$_.Trim() }

Set-Location $WorkDir
"run_id=$RunId indexes=$Indexes max_train_rows=$MaxTrainRows" | Out-File $LogPath -Encoding utf8

if ($EnableMlflow -and [string]::IsNullOrWhiteSpace($ParentRunId)) {
    $env:RF_RESEARCH_RUN_ID = "$RunId"
    $env:RF_RESEARCH_RUNNER = "run_rolling_zscore_selected_process_loop.ps1"
    $ParentRunId = python -c "import os; from research_random50_initial_features import _mlflow; m=_mlflow(); r=m.start_run(run_name='rolling_zscore_50_h4h1_' + os.environ['RF_RESEARCH_RUN_ID'] + '_top10_parent'); print(r.info.run_id); m.log_param('research_run_id', os.environ['RF_RESEARCH_RUN_ID']); m.log_param('runner', os.environ['RF_RESEARCH_RUNNER']); m.log_param('run_scope', 'top10_refit'); m.end_run()"
